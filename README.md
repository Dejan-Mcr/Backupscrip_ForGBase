...
- 脚本退出时（正常或异常），通过 `trap 'rm -f "$lock_file"' EXIT` 自动删除锁文件
...
|版本|日期|主要变更|
| ------| ------------| --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
|v4.1|2026-05-11|修复锁文件 PID 残留（head -1 + 截断写入 + trap 清理）；修复 `check_db`​ 非 local 变量（显式全局 `_GBP_DATA_DIR`​）；修复 Python 退出码混淆（错误=2，正常=1）；修正日志描述；monitor 脚本：只读锁检测兼容跨用户、realpath 路径解析、移除 `$1` 传参|
